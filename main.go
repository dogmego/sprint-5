...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	speedMean := r.meanSpeed()
	return (CaloriesMeanSpeedMultiplier*speedMean + CaloriesMeanSpeedShift) * r.Weight / MInKm * r.Duration.Hours() * MinInHours
}
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	speedMetresSec := w.meanSpeed() * KmHInMsec
	return ((CaloriesWeightMultiplier*w.Weight + (math.Pow(speedMetresSec, 2)/w.Height*CmInM)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours)
}
...
func (s Swimming) meanSpeed() float64 {
	// вставьте ваш код ниже
	floatDuration := s.Duration.Hours()
	if floatDuration == 0 {
		return 0 // чтобы избежать деления на ноль
	}
	return float64(s.LengthPool*s.CountPool) / float64(MInKm) / floatDuration
}
